...
std::set<char> &TuringMachine::additionalAlphabet() {
  return additional_alphabet_;
}
...
void TuringMachine::stateAdd() {
  finite_table_.push_back(std::map<char, Function>());
}
void TuringMachine::stateRemove() {
  if (finite_table_.size() == 1) {
...
void TuringMachine::setAlphabet(std::string main_alphabet,
                                std::string additional_alphabet) {
  resetAlphabet();
  setMainAlphabet(main_alphabet);
  setAdditionalAlphabet(additional_alphabet);

  std::vector<int> intersection;
  std::set_intersection(main_alphabet_.begin(), main_alphabet_.end(),
                        additional_alphabet_.begin(),
                        additional_alphabet_.end(),
...
void TuringMachine::setFunction(int state, char character, std::string string) {
  ...
  if (string[i] != '<' && string[i] != '>' && string[i] != '!' &&
    ...
    if (main_alphabet_.find(string[i]) == main_alphabet_.end() &&
        additional_alphabet_.find(string[i]) == additional_alphabet_.end()) {
